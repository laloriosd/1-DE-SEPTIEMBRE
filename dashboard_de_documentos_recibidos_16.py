# -*- coding: utf-8 -*-
"""Dashboard de Documentos Recibidos 16
...
# Remove the file uploader as we are now using a fixed file path
# uploaded_file = st.file_uploader("Sube tu archivo Excel de notificaciones", type=["xlsx"])

# Load and process data from the specified Excel file
try:
    # Load data from the fixed file path
    df_notificaciones = pd.read_excel('/content/nuevoDF3.xlsx')
    ...
    df_month_filtered = df_year_filtered[df_year_filtered['Month'] == selected_month]
    ...
    if selected_plaza != 'Todas':
        df_filtered = df_filtered[df_filtered['PLAZA/ENTIDAD'] == selected_plaza]
    if selected_materia != 'Todas':
        df_filtered = df_filtered[df_filtered['MATERIA'] == selected_materia]
    ...
    if not df_filtered.empty:
        ...
        # --- Mostrar visualizaciones en Streamlit en un layout de 2x2 (adjusting for no map) ---
        # We will now have 3 main visualizations: Bar, Gauge, and Pie.
        # We can arrange them in two columns, perhaps with the bar chart in the first column
        # and the gauge and pie charts stacked in the second column.
    ...
    st.error("Error: Asegúrate de que el archivo 'Latitud y longitud.xlsx' esté en la misma carpeta que el script de Streamlit, y que '/content/nuevoDF3.xlsx' exista.")
