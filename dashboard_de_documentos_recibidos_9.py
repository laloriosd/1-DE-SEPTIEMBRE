# -*- coding: utf-8 -*-
"""Dashboard de Documentos Recibidos 9
...
if uploaded_file is not None:
    ...
    try:
        ...
        if not df_filtered.empty:
            ...
            # Visualización 3: Distribución de Documentos por Plaza/Entidad (Choropleth Map)
            plaza_counts = df_filtered['PLAZA/ENTIDAD'].value_counts().reset_index()
            plaza_counts.columns = ['PLAZA/ENTIDAD', 'Count']

            # Extract State/Entity from PLAZA/ENTIDAD for mapping
            plaza_counts['State'] = plaza_counts['PLAZA/ENTIDAD'].apply(lambda x: x.split(',')[-1] if ',' in x else x)

            # Note: Choropleth maps require a GeoJSON file or built-in geometries for states.
            # Since we don't have a specific GeoJSON for Mexican states readily available in this context,
            # we will use a simplified approach with Plotly Express which has built-in country geometries.
            # However, mapping to specific states within Mexico requires more detailed data or a custom GeoJSON.
            # For this example, we will attempt to map based on the 'State' column, but accuracy may vary
            # depending on how Plotly's built-in geometries handle Mexican states. A more robust solution
            # would involve a GeoJSON of Mexican states and matching by a state ID or name.

            fig_map = px.choropleth(plaza_counts,
                                    locations='State',
                                    locationmode='country names', # Use country names to try and match states
                                    color='Count',
                                    scope='north america',
                                    color_continuous_scale='Reds',
                                    title=f"Distribución de Documentos por Estado en {selected_month}, {selected_year}",
                                    hover_name='PLAZA/ENTIDAD') # Show full Plaza/Entidad on hover
